...
final String sectionName = "Homemade";

class ListViewHomemade extends StatefulWidget {
  @override
  State<StatefulWidget> createState() {
    return _ListHomemadeState();
  }
}

class _ListHomemadeState extends State<ListViewHomemade>
    with AutomaticKeepAliveClientMixin<ListViewHomemade> {
  Future<List<HomemadeItem>> _postResult;
  ...
  Widget build(BuildContext context) {
    super.build(context);
    return _getHomemadeFutureList();
  }

  FutureBuilder<List<HomemadeItem>> _getHomemadeFutureList() {
    return FutureBuilder<List<HomemadeItem>>(
      future: _postResult,
      builder: (BuildContext context, AsyncSnapshot snapshot) {
        if (snapshot.hasData && _productInsideCartById != null) {
          List<HomemadeItem> listItems = snapshot.data;
  ...
  GridView _buildList(List<HomemadeItem> listItems) {
    ...
    return GridView.builder(
      ...
      itemBuilder: (BuildContext context, int index) {
        HomemadeItem item = listItems[index];
  ...
  Future<List<HomemadeItem>> _fetchPost() async {
    Map<String, dynamic> match = {
      "is_Visible": "1",
    };

    List<HomemadeItem> listItem = List();

    var url =
        'http://talabatakawamer.com/TalabatakAwamerApp/HomeProductSection/getAllProduct.php';
    ...
    if (response.statusCode == 200) {
      ...
      for (int i = 0; i < list.length; i++) {
        listItem.add(HomemadeItem.fromJson(list[i]));
      }
